...
import (
	...
	"github.com/skudriavtsev/caddy/caddyhttp/ibredirect"
)
...
func (p Proxy) match(r *http.Request) (Upstream, error) {
	...
	host, err := ibredirect.GetUnSuffixedDomain(r.Host, p.Suffix)
	if err != nil {
		return nil, err
	}
	uh, err := upstream.NewHost("http://" + host)
	if err != nil {
		return nil, err
	}
	upstream.Hosts = []*UpstreamHost{ uh }
	upstream.HealthCheck.Host = ""
